...
const SCENES = [
  {
	...
	*/
    ...
    spheres: [{ center: [0.0, 0.0, 2.0], radius: 0.5, material: "custom" }],
    planes: [],
    cylinders: [],
    mesh: null,
  },
...
export class Raytracer {
  ...
  async rt_pipeline_for_scene(scene) {
    ...
    shader_source_frag = shader_source_frag.replace(
      "inject-num-materials",
      `${materials.length}`
    );
    ...
    function next_object_material(mat_name) {
      ...
      uniforms[`object_material_id[${num_objects}]`] =
        material_id_by_name[mat_name];
      num_objects += 1;
    }
  ...
  save_image() {
    framebuffer_to_image_download(
      this.regl,
      this.result_framebuffer,
      `${this.scene_name}.png`
    );
  }
}
