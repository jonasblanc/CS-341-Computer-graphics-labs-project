...
function terrain_build_mesh(height_map) {
	...
	for(let gy = 0; gy < grid_height; gy++) {
		for(let gx = 0; gx < grid_width; gx++) {
			for(let gz = 0; gz < grid_deepness; gz++) {
				...
				normals[idx] = vec3.normalize([0, 0, 0], [
					-(height_map.get(gx+1, gy, gz) - height_map.get(gx-1, gy, gz)) / (2. / grid_width),
					-(height_map.get(gx, gy+1, gz) - height_map.get(gx, gy-1, gz)) / (2. / grid_height),
					-(height_map.get(gx, gy, gz+1) - height_map.get(gx, gy, gz-1)) / (2. / grid_deepness),
				])
				...
				*/
				...
				const mapped_Z = gz / grid_deepness -0.5;
	...
	for(let gy = 0; gy < grid_height - 1; gy++) {
		for(let gx = 0; gx < grid_width - 1; gx++) {
			for(let gz = 0; gz < grid_deepness - 1; gz++) {
